<div x-data="{ show_request_confirm: false, idModal: '' }" x-show="show_request_confirm"
    x-on:open-modal-new.window="show_request_confirm = true; idModal = $event.detail.idModal;"
    x-on:close-modal-new.window="show_request_confirm = false" x-on:keydown.escape.window="show_request_confirm = false"
    ...
    style="display: none;">
    <div class="bg-kuning-50 dark:bg-yellow-200 relative w-full max-w-md max-h-full inset-y-1/4" style="left: 40%">
        <div class="relative bg-kuning-50 dark:bg-yellow-200 rounded-lg shadow">
            <button x-on:click="$dispatch('close-modal-new')" type="button"
